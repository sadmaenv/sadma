...
class InferWorker(BaseWorker):
    def __init__(self, args, mac, episode_buffer, queue_center, shared_buffer, total_step, total_episode):
        super().__init__()
        self.args = args
        self.mac = mac
    ...
    def async_infer(self):
        ...
        while True:
            ...
            if terminated_bs.size > 0:
                ...
                self.total_episode += num_episode
            ...
            for idx, actor_id in enumerate(self.batch_infer["sample_id"]):
                if self.args.local:
                    ...
                    self.queue_center.put("sample_request", [actor_id, infer_bs[target_slice], infer_ts[target_slice], self.total_step.item()])
