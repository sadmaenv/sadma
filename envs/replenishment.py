...
class ReplenishmentEnv(MultiAgentEnv):
    ...
    def step(self, actions):
        ...
        if self.n_agents > 1 else np.array([reward, ]).astype(np.float32) / 1e4,
    ...
    def get_scheme(self):
        ...
        scheme = {
            ...
            "reward": {"vshape": (1,), "dtype": np.float32},
            "individual_rewards": {"vshape": (1,), "group": "agents", "dtype": np.float32},
