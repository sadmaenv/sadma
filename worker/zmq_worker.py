...
class InferClient:
    ...
    def recv_data(self):
        ...
        if (self.data_socket in sockets_poll and sockets_poll[self.data_socket] == zmq.POLLIN):
            ...
            return actions, info["total_steps"]
        else:
            return "stop", 0
...
class InferServer:
    ...
    def get_bs(self, sample_id, runner_id):
        start = self.args.env_batch_size * (sample_id * self.args.num_env_runner + runner_id)
        return np.arange(start, start + self.args.env_batch_size)
    ...
    def send_data(self):
        ...
        while not self.stop_signal:
            actor_id, bs, ts, total_steps = self.queue_center.get("sample_request")
